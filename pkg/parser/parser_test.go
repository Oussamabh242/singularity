...
func TestParse(t *testing.T) {
	...
	//
	//    byteArray := []byte{5, 0, 0, 0, 30, 0, 17, 123, 34, 113, 117, 101, 117, 101, 34, 58, 32, 34, 116, 104, 105, 115, 34, 125, 0, 9, 115, 111, 109, 101, 116, 104, 105, 110, 10}
	// 	got := Parse(byteArray)
	// 	want := Packet{
	// 		PacketType:  1,
	// 		RLenght:     uint(17+9+4),
  ...
  t.Run("parsing a Metadata only packet ", func(t *testing.T) {
    byteArray := []byte{5, 0, 0, 0, 21, 0, 17, 123, 34, 113, 117, 101, 117, 101, 34, 58, 32, 34, 116, 104, 105, 115, 34, 125, 0, 0}
    got := Parse(byteArray)
    want := Packet{
      PacketType:  5,
      RLenght:     uint(21),
